c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c     ALTERATIONS BY MATTHEW JOHN PAYNE (Dec 2009)
c     matthewjohnpayne@gmail.com
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c
c     MJP-ALTERATION#1 
c     Read body's optional parameters params(NPARAMS,nbod) from:
c     "big.in"
c     "small.in"
c     -->>The values are read in by the MIO_IN subroutine
c     -->>This in turn uses the external "user_params" file to set things up correctly
c     -->>Passing these variables around inside params(NPARAMS,nbod) required the
c     addition of "params" to the various subs & their declared vars.
c     -->> There is also an alteration to the routine that dumps the data to the
c          .dmp & .tmp files
c
c     Use of params within the code
c     -->> See the USER_INITIALISATION.F file
c
c
c
c     MJP-ALTERATION#2 Altered the close encounter & collision routines... 
c     (a) There is now an OPTION to print out the details of close encounters
c         -->> I find that the disk access can be very time consuming
c     (b) There is now an option to choose whether to record close approaches between
c         Big:All or just Big:Big
c     (c) Whether to consider collisions with the central star in mce_stat & mce_box
c         (AT PRESENT I CAN'T REMEMBER WHY THIS WAS USEFUL, BUT IT WAS)
c     (d) I Also changed the mio_ce routine so that it stores details of 
c         10,000 objects instead of just 100 before it writes out.
c
c
c
c     MJP-ALTERATION#3 Record some details to user.out as the integration 
c                      progresses
c
c
c     MJP-ALTERATION#4 Add in various routines to mfo_user via the external user_forces routine.
c                      This allow the modelling of additional (often non-conservative)
c                      forces
c
c
c     MJP-ALTERATION#5 Alter the code to allow user-defined forces to be used with
c                      the binary (wb & cb) integrators.
c                      Apart from turning off the warning flag in mio_err, this probably just involves 
c                      correctly converting the coordinate system.
c
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
...
c
      ...
      real*8 params(NPARAMS,NMAX),rphys(NMAX)
      integer initial(4)
      character*8 id(NMAX)
...
c
c     TIME STUFF
c      real etime          ! Declare the type of etime()
...
c TIME STUFF...
      ...
      write(1,*) 'Start:  user=', elapsed(1),
     &         ' + system=', elapsed(2), ' => totaltime=', totaltime
      close(unit=1)
c
c INITIALIZE ALL OF THE STUFF REQUIRED FOR THE GAS-DISK POTENTIAL
c      namelist_file  = 'dg_input.nml'
c      call read_params(namelist_file)
      call initialize_gravity()
      call make_disk_model()
      call get_spherical_moments()
      open (unit=1,file='user.out',access='append')
      write(1,*) 'use_grid=',use_grid,', nr=',nr
      close(unit=1)
c
c
...
c If this is a new integration, integrate all the objects to a common epoch.
  ...
  20    open (23,file=outfile(3),status='old',access='append',err=20)
     ...
     %    rceh,stat,id,epoch,ngf,opt,ngflag,params,lmem,mem,initial,
     %    rphys)
...
c Do a final data dump
     ...
     %  id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,params,initial,rphys)
c
...
c TIME STUFF...
      Call etime(elapsed,totaltime)
      open (unit=1, file='user.out', access='append')
      write(1,*) 'End:  user=', elapsed(1),
     &         ' + system=', elapsed(2), ' => totaltime=', totaltime
      close(unit=1)
!      open (unit=1, file='user.out', access='append')
!      write(1,*) 'DG_A(GRID):  timertotal=', timertotal
!      close(unit=1)
c
c------------------------------------------------------------------------------
c
...
c
     ...
     %                     lmem,mem,h0,algor,msum,initial,s,rphys)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c
c Local
      integer i,j,k,ngflag,opt
      real*8 ngf(4,nbod)
      real*8 xh(3,nbod),vh(3,nbod)
      real*8 sum_new_h_mv(3),scaled_sum_new_h_mv(3),dt_1,dt_msum_1
c
c------------------------------------------------------------------------------
c
c Need to keep this default/reset in here, as the addition to the central 
c  forces is done elsewhere. If you don't reset to zero you just end up adding
c  forces at each step
      do j = 2, nbod
        a(1,j) = 0.d0
        a(2,j) = 0.d0
        a(3,j) = 0.d0
...
c
c
c If the call to mfo_user has occured while in cb or wb mode, need to 
c convert to heliocentric coords
      if (algor.eq.11) then 
         call mco_cb2h (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,
     %        opt,params,lmem,mem,algor,initial,s,rphys)
      end if
      if (algor.eq.12) then 
         call mco_wb2h (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,
     %        opt,params,lmem,mem,algor,initial,s,rphys)
      end if
c
c
c Call the newly defined routine (external file) which contains all 
c     the additional user-defined forces
      if(algor.eq.11.or.algor.eq.12) then 
         call user_forces(time,nbod,nbig,m,xh,vh,a,params,
     %        lmem,mem,h0,algor,initial,s,rphys)
      else
         call user_forces(time,nbod,nbig,m,x,v,a,params,
     %        lmem,mem,h0,algor,initial,s,rphys)
      end if
c
c Need to convert accelerations for the cb & wb cases
      if(algor.eq.11.or.algor.eq.12) then 
c (i) Sum the masses of interest (inc/exc m_B=m(2))
c N.B. have pre-calculated mass sum over the non-binary bodies
         msum = msum + m(1) 
         if(algor.eq.11) then
            msum = msum + m(2)
         end if 
         dt_1 = 1.d0 / h0
         dt_msum_1 = dt_1 / msum
c (ii) Calculate the Summation of __new__, __heliocentric__ momentum, and then divide by the total mass of interest (and the time period)
c      Note that the momentum of the central star (1) is automatically = 0, and that the mom of (2) is only added if this is a c-b 
         do k = 1, 3
         scaled_sum_new_h_mv(k)= 0.d0
         sum_new_h_mv(k)= 0.d0
         do j = 3, nbod
         sum_new_h_mv(k)= sum_new_h_mv(k) + m(j)*(vh(k,j) + h0*a(k,j))
         end do 
         if(algor.eq.11) then
            sum_new_h_mv(k)=sum_new_h_mv(k)+m(2)*(vh(k,2) + h0*a(k,2))
         end if
         scaled_sum_new_h_mv(k)= dt_msum_1 * sum_new_h_mv(k)
         end do
c (iii) Bring it all together
         do k = 1, 3
            do j = 3, nbod
               a(k,j) = a(k,j) - scaled_sum_new_h_mv(k)
               a(k,j) = a(k,j) + (vh(k,j)-v(k,j)) * dt_1
            end do
         end do
c         
      end if
...
c Input/Output
      ...
      real*8 params(NPARAMS,nbod),rphys(nbod)
      integer initial(4)
      character*8 id(nbod)
      character*80 outfile(3),dumpfile(4),mem(NMESS)
c
c Local
      ...
      real*8 rce(NMAX),rcrit(NMAX),a(NMAX)
...
c Update the data dump files
     ...
     %    rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,
     %    id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,params,initial,
     %    rphys)
        tdump = time
      end if
c
...
c Save the current coordinates and velocities
     ...
     %  params,lmem,mem,algor,initial,s,rphys)
c
...
c Remove lost objects, reset flags and recompute Hill and physical radii
     ...
     %    id,mem,lmem,outfile(3),itmp,params,initial,rphys)
        dtflag = 1
...
c Check for collisions
     ...
     %     jhit,thit,dhit,algor,ngf,ngflag,params,lmem,mem,initial,
     %     s,rphys)
c     
...
c
     ...
     %  rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,
     %  stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,lmem,onestep,
     %  coord,bcoord,params,initial,rphys)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c Local
      ...
      real*8 rce(NMAX),rcrit(NMAX),epoch(NMAX)
...
c Check for collisions with the central body
     ...
     %  nhit,jhit,thit,dhit,algor,ngf,ngflag,params,lmem,mem,initial,
     %  s,rphys)
c
c If something hit the central body, restore the coords prior to this step
     ...
     %    ngflag,opt,params,lmem,mem,algor,initial,s,rphys)
        time = time - h0
c
...
c------------------------------------------------------------------------------
c
c MJP-ALTERATION#2(c) #####################################################
c Is sometimes useful to have a record of the close encounters with the central star
c BUT SURELY THIS IS RECORDED ELSEWHERE?
c THIS SEEMED TO BE OF USE WHEN USING MERCURY TO LOOK AT CLUSTERS...
c
      if (CENTRALSTARCOLLISIONS.eq.1) then 
         startindex = 1    
      else 
         startindex = 2    
      end if
c
c MJP-ALTERATION#2(c) - END ###############################################
c
      do j = startindex, nbod
...
c
c
c MJP-ALTERATION#2(a) #######################################################
c Add in logic to turn off the recording of close encounters
      if (RECORDCLOSEENCOUNTERS.eq.1) then
c
...
c MJP-ALTERATION#2(a) - END OF ALTERATION ################################
c
c------------------------------------------------------------------------------
c
      return
...
c
      ...
      if (BIGBIGCOLLISIONSONLY.eq.1) then 
         endindex   = nbig 
      else 
         endindex   = nbod 
      end if
c
c MJP-ALTERATION#2(b & c) - END ###############################################
c
...
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) then  
           msum = 0.d0
           call mfo_user (time,jcen,nbod,nbig,m,xh,vh,
     %    ausr,params,lmem,mem,h,algor,msum,initial,s,rphys)
        end if
...
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) then 
           msum=0.d0
           call mfo_user (time,jcen,nbod,nbig,m,x,v,ausr,
     %    params,lmem,mem,h,algor,msum,initial,s,rphys)
        end if
...
c
     ...
     %  rphys,rcrit,ngf,stat,dtflag,ngflag,opt,nce,ice,jce,force,params,
     %  lmem,mem,algor,initial)
c
      implicit none
      include 'mercury.inc'
...
c Input/Output
      ...
      integer nce, ice(nce), jce(nce), algor
...
c
      real*8 r2old(NMAX), r2veryold(NMAX),r2new(NMAX)
      real*8 binary_r2old(NMAX), binary_r2veryold(NMAX)
      real*8 binary_r2new(NMAX), old_potn(NPARAMS)
      real*8 delta_addn_potn
      save r2old,r2veryold,r2new
c
c------------------------------------------------------------------------------
c
      tol2 = tol * tol
...
c V^2 for velocity).
      do k = 2, nbod
         r2veryold(k) = r2old(k)          ! Only required in Nagasawa
         r2old(k)     = r2new(k)          ! Only required in Nagasawa
         r2new(k) = x0(1,k)*x0(1,k) + x0(2,k)*x0(2,k) + x0(3,k)*x0(3,k)  ! Required here & in Nagasawa
c         tmp1     = x0(1,k)*x0(1,k) + x0(2,k)*x0(2,k) + x0(3,k)*x0(3,k)
         tmp2     = v0(1,k)*v0(1,k) + v0(2,k)*v0(2,k) + v0(3,k)*v0(3,k)
         xscal(k) = 1.d0 / r2new(k)
         vscal(k) = 1.d0 / tmp2
c
         old_potn(k) = params(18,k)       ! This is required for the disk grav potn 
      end do
c
c
c
c
c If required, implement the simple Nagasawa tidal damping kick 
c  - Note that I am putting it here because the decision to kick at pericenter
c    requires knowledge of whether you are at pericenter, and this is trivial to calculate
c    if you know the value of the next radial location. I therefore use this and implement
c    the kick before the next step...
c
c     N.B. For the binary regime this implies the binaries will interact. 
c     But we probably don't want dynamical tides to be considered in the long term.
c     However, the TIDALCUT criterion should effectively take care of this.
      if(T4NAGASAWATIDALSWITCH.eq.1.or.T4NAGASAWABINARYSWITCH.eq.1) then
         do k = 2, nbig                    !  ---  Only doing this over nbig (but record the info above for nbod)
            if (r2new(k).lt.TIDALCUT) then    !  ---  Checking the close approach criteria
             if (r2veryold(k).gt.r2old(k).and.r2new(k).gt.r2old(k)) then ! Implies it went through a minimum
                call t4nagasawatidal(time,nbig,mass,x0,v0,params,h0,k)
             end if
          end if
       end do
      end if
c
      if(T4NAGASAWABINARYSWITCH.eq.1) then
         do k = 3, nbig                                                  !  ---  Only doing this over nbig
            binary_r2veryold(k) = binary_r2old(k)
            binary_r2old(k)     = binary_r2new(k)
            binary_r2new(k) = x0(1,k)**2 + x0(2,k)**2 + x0(3,k)**2
            if (binary_r2new(k).lt.TIDALCUT) then                        ! Checking the close approach criteria
               if (binary_r2veryold(k).gt.binary_r2old(k)) then          !   (a) -|
                if (binary_r2new(k).gt.binary_r2old(k)) then             ! + (b) -| - Implies went through a minimum
                call t4nagasawabinary(time,nbig,mass,x0,v0,params,h0,k)
                end if
             end if
          end if
       end do
...
c - MJP this is calling mfo_all, which in turn calls mfo_grav
c - Uses positions to calculate force between objects & updates acceleration arrays
      call force (time,jcen,nbod,nbig,mass,x0,v0,s,rcrit,a0,stat,ngf,
     %  ngflag,opt,nce,ice,jce,params,lmem,mem,algor,h0,initial,rphys)
c
 100  continue
c
c For each value of N, do a modified-midpoint integration with 2N substeps
      do n = 1, 8
        h = h0 / (2.d0 * float(n))
        h2(n) = .25d0 / (n*n)                ! Doesn't need to be done in the loop, just put it in the initialization
        hx2 = h * 2.d0
c
        do k = 2, nbod
...
c           write(1,*) 't:x(xyz)=',time,' ',x(1,k),' ',x(2,k),' ',x(3,k)
c           close(unit=1)  
...
c
     ...
     %      ngf,ngflag,opt,nce,ice,jce,params,lmem,mem,algor,h0,initial,
     %      rphys)
          do k = 2, nbod
          ...
          call force (time,jcen,nbod,nbig,mass,x,v,s,rcrit,a,stat,ngf,
     %      ngflag,opt,nce,ice,jce,params,lmem,mem,algor,h0,initial,
     %      rphys)
          do k = 2, nbod
...
c Maximum relative position and velocity errors (last D term added)
     ...
     %             d(6,k,1)*d(6,k,1) )
              ...
              if (DISKGRAVITYSWITCH.eq.1) then
                 delta_addn_potn = old_potn(k) - params(18,k)            !!! Added in for the benefit of the non-Keplerian potn work
                 delta_addn_potn = sqrt(delta_addn_potn*delta_addn_potn) !!! Added in for the benefit of the non-Keplerian potn work
!                 errmax = max(errmax, tmp1*xscal(k),                     !!! The contribution to the error component from the potn/posn now ...
!     %                (tmp2*vscal(k) - delta_addn_potn)  )               !!! ... has the absolute change in the perturbing potn subtracted out 
                 errmax = max(errmax, tmp1*xscal(k), tmp2*vscal(k))      !!! This is the original logic as used by the BS1 routine by Chambers
                 
!                 open (unit=1,file='user.out',access='append')
!                 write(1,*) 'LOOP...k,n...t1,t2,t1*x,t2*v,delta,',
!     %                't2*v-delta',
!     %                k,n,tmp1,tmp2,tmp1*xscal(k),tmp2*vscal(k),
!     %                delta_addn_potn,(tmp2*vscal(k) - delta_addn_potn),
!     %                'd(3,2,)=',d(1,k,1),d(3,k,1),d(3,k,1)
!                 close(unit=1)  
...
c
c --->>> MJP Need to do something about this error estimation
c --->>> Basis above uses only Keplerian potential, blah, blah blah
c
c If error is smaller than TOL, update position and velocity arrays, and exit
c           open (unit=1,file='user.out',access='append')
c           write(1,*) 'EXIT...time, errmax,  Tol,  h0 & n= ',time,' & ',
c     %          errmax,tol2,h0,n
c           close(unit=1)  
...
!                    if (k.eq.2.or.k.eq.3) then 
!                       open (unit=1,file='user.out',access='append')
!                       write(1,*) 'k,j,n,x0(1,k)',k,j,n,x0(1,k)
!                       close(unit=1)  
!                    end if                 
                 end do
              end do
...
c
c Input/Output
      integer nbod, nbig, opt(8), stat(nbod), dtflag, ngflag, algor
      real*8 time,h0,hdid,tol,jcen(3),mass(nbod),x0(3,nbod),v0(3,nbod)
      real*8 s(3,nbod),ngf(4,nbod),rphys(nbod),rcrit(nbod)
      real*8 params(NPARAMS,nbod)
      integer initial(4)
...
c
     ...
     %  outfile,mem,lmem,params,initial)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c Local
      ...
      real*8 angf(3,NMAX),ausr(3,NMAX),msum
      external mfo_hkce
c
c------------------------------------------------------------------------------
c
...
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) then 
           msum = 0.d0
           call mfo_user (time,jcen,nbod,nbig,m,x,v,ausr,
     %   params,lmem,mem,h0,algor,msum,initial,s,rphys)
        end if 
        if (ngflag.eq.1.or.ngflag.eq.3) call mfo_ngf (nbod,x,v,angf,ngf)
      end if
c
...
c If objects had close encounters, advance H_K using Bulirsch-Stoer instead
     ...
     %    jxvclo,outfile,mem,lmem,mfo_hkce,params,initial)
      end if
c
c Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
c
     ...
     %  ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,ixvclo,
     %  jxvclo,outfile,mem,lmem,force,params,initial)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c
     ...
     %  params,lmem,mem,algor,initial,s)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c Calculate forces at the start of the sequence
     ...
     %  ngflag,opt,nce,ice,jce,params,lmem,mem,algor,t,initial,rphys)
c
...
c Calculate forces at the current substep
     ...
     %      ngf,ngflag,opt,nce,ice,jce,params,lmem,mem,algor,t,initial,
     %      rphys)
c
...
c
c Local
      integer j,k,nce,ice(NMAX),jce(NMAX),ce(NMAX),iflag,isub,nsub
      real*8 a(3,NMAX),hby2,hrec,x0(3,NMAX),v0(3,NMAX),mvsum(3),temp
      real*8 mbin,hbin,gmbin,hsub,tmp0,tmp1,tmp2,abin,amin,msum
      external mfo_hkce
      real*8 a_usr(3,NMAX)
c
c------------------------------------------------------------------------------
...
c
c MJP-ALTERATION#5 ##################################################         
c Want to use my own forces where required.                           
        msum = 0.d0
        do j = 3, nbod
           msum = msum + m(j)
           do k =1,3
...
c If required, apply non-gravitational and user-defined forces                
        if (opt(8).eq.1) call mfo_user (time,jcen,nbod,nbig,m,x,v,a_usr,      
     %   params,lmem,mem,h0,algor,msum,initial,s,rphys)                                                    
c MJP-ALTERATION#5 - END ############################################
c
        dtflag = 2
      end if
c
...
c Advance direct-interaction Hamiltonian for H/2
      do j = 3, nbod
c MJP-ALTERATION#5 ##################################################  
c Added in the "a_usr " accelarations
         do k = 1,3
            v(k,j) = v(k,j)  +  hby2 * (a(k,j) + a_usr(k,j))
         end do
c MJP-ALTERATION#5 - END ############################################
      end do
c
...
c
c Local
      integer j,k,nce,ice(NMAX),jce(NMAX),ce(NMAX),iflag
      real*8 a(3,NMAX),hby2,hrec,x0(3,NMAX),v0(3,NMAX),mvsum(3),temp
      real*8 msum,gmbin,hbin,a_usr(3,NMAX)
      external mfo_hkce
c
c------------------------------------------------------------------------------
c
...
c also the effective central mass and timestep for the binary Kepler drift
      if (dtflag.ne.2) then
        if (dtflag.eq.0) hrec = h0
        msum = 0.d0
        call mfo_wb (jcen,nbod,nbig,m,x,rcrit,a,stat,msum)
        dtflag = 2
...
c MJP-ALTERATION#5 ##################################################         
c Want to use my own forces where required.          
c Note that am allowing a force to be applied to the binary
c However the decision to implement / not implement is taken in mfo_user                                
        do j = 2, nbod
           do k =1, 3
              a_usr(k,j) = 0.d0
           end do
...
c Input/Output
      ...
      real*8 params(NPARAMS,nbod),rphys(nbod)
      integer initial(4)
      integer lmem(NMESS), algor
      character*80 mem(NMESS)
      real*8 h0
c
c Local
      integer j
      real*8 acor(3,NMAX),acen(3),msum
c
c------------------------------------------------------------------------------
c
...
c Include user-defined accelerations if required
      if (opt(8).eq.1) then
         msum = 0.d0
         call mfo_user (time,jcen,nbod,nbig,m,x,v,acor,params,lmem,mem,
     %        h0,algor,msum,initial,s,rphys)
         do j = 2, nbod
          a(1,j) = a(1,j) + acor(1,j)
          a(2,j) = a(2,j) + acor(2,j)
          a(3,j) = a(3,j) + acor(3,j)
...
c------------------------------------------------------------------------------
c
      subroutine mfo_wb (jcen,nbod,nbig,m,x,rcrit,a,stat,msum)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c Local
      ...
      real*8 aobl(3,NMAX), acen(3), xs(3), xsm3, xxs(3)
c
c------------------------------------------------------------------------------
c
c Initialize accelerations to zero
...
c chosen by the user. Close encounter details are stored until either 10,000  
c (# THIS USED TO BE 100 -->> MJP-ALTERATION#2(d))
...
c
c MJP-ALTERATION#2(a) ######################################################
c Turn off the writing to the close-encounter file "ce.out" if desired
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      if (RECORDCLOSEENCOUNTERS.eq.1) then
c
...
c If required, output the stored close encounter details
      if (nstored.ge.10000.or.ceflush.eq.0) then    
...
c MJP-ALTERATION#2(a) - END ################################################
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
c
c
c
c -->> Note that these two subsections (Above & Below) are unrelated, 
c      and could/should be split in two for clarity
c
c
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
c
     ...
     %  stat,id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,params,initial,
     %  rphys)
c
      implicit none
      include 'mercury.inc'
...
c     MJP-ALTERATION#1 #########################################
            write (31,312) params(1,j),params(2,j),params(3,j)
            write (31,312) params(4,j),params(5,j),params(6,j)
            write (31,312) params(7,j),params(8,j),params(9,j)
            write (31,312) params(10,j),params(11,j)
c     MJP-ALTERATION#1 - END ###################################
          enddo
          close (31)
        end do
c
...
c Input/Output
      ...
      real*8 params(NPARAMS,NMAX),rphys(nbod)
      integer initial(4)
      character*80 outfile(3),dumpfile(4), mem(NMESS)
      character*8 id(NMAX)
c
...
c     MJP-ALTERATION#1 ##################################################
     ...
     %      params(1,nbod),params(2,nbod),params(3,nbod),params(4,nbod),
     %      params(5,nbod),params(6,nbod),params(7,nbod),params(8,nbod),
     %      params(9,nbod),params(10,nbod),params(11,nbod),
     %      params(12,nbod),params(13,nbod),params(14,nbod),
     %      params(15,nbod),params(16,nbod),params(17,nbod),
     %      params(18,nbod)
        else
...
c
c Convert the various parameters to the correct units for MERCURY
...
c
c
c MJP-ALTERATION#1 Define the body's optional additional parameters, params(j,nbod)
      call user_initialisation (nbod,nbig,m,x,v,rho,params,initial,
     %     s,rphys)
c
c------------------------------------------------------------------------------
c
...
c
c MJP-ALTERATION#2(a) #########################################################
c Turn off the close-encounter file "ce.out" as desired
        if (RECORDCLOSEENCOUNTERS.eq.1) then
c
        do j = 1, 2
...
c MJP-ALTERATION#3 - Write out the masses to user.out file #################
      if (USEROUTPUT.eq.1) then 
c
c     Get the r,v & a==semi (note use of Chambers' routine mco_x2a).
c      do k = 2, nbod
c            gm = m(1) + m(k)
c            call mco_x2a (gm,xh(1,k),xh(2,k),xh(3,k),
c     %           vh(1,k),vh(2,k),vh(3,k),semi(k),r,v2)
c      end do
c
         open (unit=1, file='user.out', access='append')
         do k = 2, nbig
            write(1,*) 't(yr)= ',time/365.25,' k=',k,
     %           ' m(/Sun)=',m(k)/K2,' algor= ',algor
         end do
         close(unit=1)
c
      end if
c MJP-ALTERATION#3 - END ###################################################
c
c------------------------------------------------------------------------------
c
      return
...
c
     ...
     %  id,mem,lmem,outfile,nelim,params,initial,rphys)
c
      implicit none
      include 'mercury.inc'
      include 'user_parameters.inc'
...
c
c Local
      integer i, j, k, l, nbigelim, elim(NMAX+1)
c
c------------------------------------------------------------------------------
c
...
c
     ...
     %  rho,rceh,stat,id,epoch,ngf,opt,ngflag,params,lmem,mem,initial,
     %  rphys)
c
      implicit none
      include 'mercury.inc'
...
c
     ...
     %      rcrit,ngf,stat,raflag,ngflag,opt,nce,ice,jce,mfo_all,params,
     %      lmem,mem,initial)
          time = time + hdid
        end do
        raflag = 1
